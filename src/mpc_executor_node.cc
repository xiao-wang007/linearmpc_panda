...
namespace MPCControllers
{
    ...
    MPCExecutorNode::MPCExecutorNode()
    {
        ...
        mpc_sol_sub_ = nh_.subscribe("/mpc_sol_in", 1, &MPCExecutorNode::mpc_sol_callback, this);
        ...
        u_cmd_upsampled_pub_ = nh_.advertise<std_msgs::Float64MultiArray>("/upsampled_u_cmd_out", 1);
        ...
        ROS_INFO("MPCExecutorNode initialized with u_cmd_ref before the first solution is ready. \n");
    ...
    void MPCExecutorNode::mpc_sol_callback(const linearmpc_panda::StampedFloat64MultiArray::ConstPtr& msg) 
    {
        ...
        // Ensure the dimensions match the expected sizes
        assert(rows_ == nu_ && cols_ == Nt_ && "MPC solution dimensions mismatch in executor node!");
        ...
        u_sol_ = Eigen::Map<const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>>(
        ...
        current_time_ = (t_now - mpc_t_start_).toSec();
    ...
    {        
        ...
        // std::cout << "checking in MPCExecutorNode::publish_upsampled_command()..." << std::endl;
        std::lock_guard<std::mutex> lock(sol_spline_mutex_);
        ...
        u_cmd_now_ = u_cmd_spline_.value((t_now - mpc_t_start_).toSec());
        std::cout << "t_now: " << t_now.toSec() << std::endl;
        std::cout << "elapsed time: " << (t_now - mpc_t_start_).toSec() << std::endl;
        std::cout << "u_cmd_now_: \n" << u_cmd_now_.transpose() << std::endl;
    ...
    void MPCExecutorNode::run() 
    {
        ...
        {
            nh_.setParam("/mpc_executor_ready", true);  // Global flag
            ROS_INFO("MPC executor is ready!");
        }
