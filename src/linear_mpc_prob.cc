...
namespace MPCControllers{
													  ...
													  const Eigen::VectorXd dudt_low)
	{
		...
		if (u_entries_.size() == 0)
		{
			lb_.head(nx_) = init;
			ub_.head(nx_) = init;
			std::cout << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" << std::endl;
		} else {
		  														 ...
		  														 double t_now_relative) 
    {
		...
		std::cout << "[LinearMPCProb] ts_mpc: " << ts_mpc.transpose() << std::endl;
